 ...
 * on 2020/7/28
 *
 * @Link https://github.com/chenshouyin/CustomProgress
 */
public class RoundProgress extends CustomHorizontalProgresNoNum {


    private static final int DEFAULT_PROGRESS_RADIO = 50;//圆的半径
    private static final int DEFAULT_PROGRESS_UNREACH_PAINT_STROKEN = 2;//未走完进度的宽度
    private static final int DEFAULT_PROGRESS_REACH_PAINT_STROKEN = 5;//走完进度的宽度
    private static final int DEFAULT_PROGRESS_REACH_START_DGREE = 0;//走完进度 圆弧 默认从多少度开始
    private static final int DEFAULT_PROGRESS_REACH_END_DGREE = 360;
    private static final boolean DEFAULT_PROGRESS_HAS_NUM = false;//是否带进度数字显示
    ...
    public RoundProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mTypeface = Typeface.createFromAsset(getContext().getAssets(), "num.ttf");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    protected void getStyleabletAttr(AttributeSet attrs) {
        ...
        mCustomCircleRadio = (int) typedArray.getDimension(R.styleable.CustomCircleProgresStyle_CustomCircleRadio, dp2px(getContext(), DEFAULT_PROGRESS_RADIO));
        mCustomCircleUnReachPaintStroken = (int) typedArray.getDimension(R.styleable.CustomCircleProgresStyle_CustomUnReachPaintStroken, dp2px(getContext(), DEFAULT_PROGRESS_UNREACH_PAINT_STROKEN));
        mCustomCircleReachPaintStroken = (int) typedArray.getDimension(R.styleable.CustomCircleProgresStyle_CustomReachPaintStroken, dp2px(getContext(), DEFAULT_PROGRESS_REACH_PAINT_STROKEN));
        mCustomUnReachStart = typedArray.getInteger(R.styleable.CustomCircleProgresStyle_CustomUnReachStart, DEFAULT_PROGRESS_REACH_START_DGREE);
        mCustomUnReachEnd = typedArray.getInteger(R.styleable.CustomCircleProgresStyle_CustomUnReachEnd, DEFAULT_PROGRESS_REACH_END_DGREE);
        mCustomHasNum = typedArray.getBoolean(R.styleable.CustomCircleProgresStyle_CustomHasProgressNum, DEFAULT_PROGRESS_HAS_NUM);
    ...
    public void setHorizontalProgressTextColor(int textColor) {
        HorizontalProgresTextColor = textColor;
        invalidate();
    }

    public void setHorizontalProgresReachColor(int color) {
        HorizontalProgresReachColor = color;


    }

    public void setHorizontalProgresUnReachColor(int color) {
        HorizontalProgresUnReachColor = color;
        invalidate();
    }
