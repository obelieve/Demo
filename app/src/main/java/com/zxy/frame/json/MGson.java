...
public class MGson {
    ...
    public static Gson newGson() {
        ...
        try {
            ...
            //注册反射对象的处理器
            gsonBuilder.registerTypeAdapterFactory(new TypeAdapterReflectiveFactory(new ConstructorConstructor(val), FieldNamingPolicy.IDENTITY, Excluder.DEFAULT));
            //注册集合的处理器
            gsonBuilder.registerTypeAdapterFactory(new TypeAdapterCollectionFactory(new ConstructorConstructor(val)));
        } catch (Exception e) {
            e.printStackTrace();
        }
