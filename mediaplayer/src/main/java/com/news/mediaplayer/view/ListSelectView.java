...
public class ListSelectView extends FrameLayout {
    ...
    public void loadData(IListSelectView selectView, List<IListSelectViewData> list, int selectType) {
        loadData(null, new LinearLayoutManager(getContext()), selectView, list, -1, selectType);
    }
    ...
    public void loadData(RecyclerView.LayoutManager layoutManager, IListSelectView selectView, List<IListSelectViewData> list, int selectType) {
        loadData(null, layoutManager, selectView, list, -1, selectType);
    }
     ...
     * @param selectView
     * @param list
     * @param curSelectedPosition
     * @param selectType          ListSelectView#SINGLE_TYPE,ListSelectView#MULTI_TYPE
     */
    public void loadData(RecyclerView.ItemDecoration[] decorations, RecyclerView.LayoutManager layoutManager, IListSelectView selectView, List<IListSelectViewData> list, int curSelectedPosition, int selectType) {
        mLayoutManager = layoutManager;
        mIListSelectView = selectView;
        mCurSelectedPosition = curSelectedPosition;
        mSelectType = selectType;
        mList = list != null ? list : new ArrayList<IListSelectViewData>();
            ...
            public void onItemClick(View view, IListSelectViewData data, int position) {
                if (mSelectType == SINGLE_TYPE) {
                    if (mCurSelectedPosition != position) {
                        ...
                        if (mCallback != null) {
                            mCallback.onSingleSelected(data, mCurSelectedPosition);
                        }
                    }
    ...
    public interface Callback {
        ...
        void onSingleSelected(IListSelectViewData data, int position);
    }
